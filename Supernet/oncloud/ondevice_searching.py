...
# Dataset parameters
parser.add_argument('--data_dir', metavar='DIR',default='/home/junjie/1_dataset/ImageNet/val',
...
parser.add_argument('--model_path', metavar='DIR',default='weights/resnet500epoch0.pth',
...
parser.add_argument("--batch_size_for_lat", default=32, type=int)
...
parser.add_argument('--dirichlet_alpha', type=float, default=None,
                    help='BatchNorm momentum override (if not None)')
parser.add_argument('--test_device',  default='3090', type=str,
...
def main():
    ...
    if args.test_effi:
        loader_eval = torch.utils.data.DataLoader(dataset_eval, batch_size=args.batch_size, shuffle=False, num_workers=8)
    ...
    if args.data_aware and args.randomlysample:
        ...
        results = []
        search = torchvision.datasets.ImageFolder(root='/home/junjie/1_dataset/AdaptiveNet_CAVE/imagenet/SearchForDevice3k/',transform=data_transform)
        test = torchvision.datasets.ImageFolder(root='/home/junjie/1_dataset/AdaptiveNet_CAVE/imagenet/ValForDevice3k/',transform=data_transform)
        ...
        for alpha in [args.dirichlet_alpha]:
            ...
            loader_search = torch.utils.data.DataLoader(search_set, batch_size=args.batch_size, shuffle=False, num_workers=8)
            loader_test = torch.utils.data.DataLoader(test_set, batch_size=args.batch_size, shuffle=False, num_workers=8)
    ...
    dataset_eval = torchvision.datasets.ImageFolder(root='/home/junjie/1_dataset/AdaptiveNet_CAVE/imagenet/SearchForDevice3k',transform=data_transform)
    loader_eval = torch.utils.data.DataLoader(dataset_eval, batch_size=args.batch_size, shuffle=False, num_workers=8)
    dataset_test = torchvision.datasets.ImageFolder(root='/home/junjie/1_dataset/AdaptiveNet_CAVE/imagenet/ValForDevice3k/',transform=data_transform)
    loader_test = torch.utils.data.DataLoader(dataset_test, batch_size=args.batch_size, shuffle=False, num_workers=8)
    ...
    for ratio in range(1):
        print("--------------", 0.95-0.05*ratio, "---------------")
        finder = EvolutionFinder(batch_size=args.batch_size, branch_choices=model.block_choices,
                                 time_budget=(0.95-0.05*ratio)*baseline_latency, searching_times=1, lats=lats, modeltype=modeltyp, pruned=args.pruned, pruned_points=pruned_points, propulation_size=5)
        print("开始搜索")
        _, best_info = finder.evolution_search(model, validate, loader_eval, args, loss_fn)
        test_acc, test_lat = validate(model, loader_test, best_info, args, loss_fn, lats)
