...
#                     help='Name of dataset to train (default: "coco"')
parser.add_argument('--dataratio', type=float, default=1, metavar='ratio of dataset used',
                    help='训练和评估的数据集比例')
...
def validate(subnet_index,model, loader, args, subnet, evaluator=None, log_suffix='', lats=None):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(tqdm(loader)):
            # img_list=['000000000139','000000000785','000000001268','000000001503','000000001818','000000002157','000000002532','000000003156','000000003845','000000004765']
            # if cx<len(img_list):
            ...
            #         save_path = os.path.join('output/img/', f"{img_list[cx]}_epoch_29.jpg")
    ...
    # 保存预测为JSON
    # with open('predictions'+str(subnet_index)+'.json', 'w') as f:
    #     json.dump(results, f)
...
def main():
    ...
    is_evaluate = True  # joy eval
    if is_evaluate:
        for i in range(len(subnets)):
            # print("subnet=", subnet)
            s_time = time.time()
            with suppress_stdout():
                mAP, lat = validate(i,model, loader_eval, args, subnets[i], evaluator, lats=latencys)
            mAP = round(mAP, 5)
            time_used = round(time.time() - s_time, 3)
            latency_list.append(time_used)
            acc_list.append(mAP)
            print(f'===========mAP: {mAP}, latency: {time_used}, subnet_architecture: {subnets[i]}============')
        print('acc\tlatency')
