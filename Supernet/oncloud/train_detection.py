...
from tool.JoyTool import send_post,write_list_to_txt
...
parser.add_argument('--dataratio', type=float, default=0.2, metavar='ratio of dataset used',
                    help='训练和评估的数据集比例')
...
# parser.add_argument('--backbonepath', default='weights/resnet500epoch0.pth', metavar='DIR', help='path to dataset')
parser.add_argument('--headpath', default='checkpoint/pretrained_weight/resdet50_416-08676892.pth', metavar='DIR',
                    help='path to dataset')
parser.add_argument('--recovery', action='store_true', default=True,
                    help='从历史训练的模型中恢复')
parser.add_argument('--recoverypath', default='checkpoint/SuperNet_epoch_29_mAP_21_19.pth', metavar='DIR',
                    help='path to dataset')
# parser.add_argument('--recoverypath', default='checkpoint/SuperNet_epoch_28_mAP_34_90.pth', metavar='DIR',
#                     help='path to dataset')
...
parser.add_argument('--min-lr', type=float, default=1e-3, metavar='LR',
...
parser.add_argument('--distillhead', action='store_true', default=False)
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with autocast('cuda'):
            # oncloude/effdet/efficientdet.py
            subnet = model.model.generate_random_subnet(sample_policy)
            # print(model.model)
            while oncloud.no_new_detection_subnet(subnet):
                subnet = model.model.generate_random_subnet(sample_policy)
            # print("train subnet={}".format(subnet))
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.local_rank == 0:
                # print("losses_m={}".format(losses_m.avg))
                loss_list.append(round(losses_m.avg,5))
                logging.info(
                    'Train: {}/{} [{:>4d}/{} ({:>3.0f}%)]  '
                    'Loss: {loss.val:>9.6f}  '
                    'LR: {lr:.3e}'.format(
    ...
    write_list_to_txt(loss_list,sample_policy+"_loss_epoch_"+str(epoch))
...
def validate(model, loader, args, evaluator=None, log_suffix=''):
    ...
    for subnet in subnets:
        print(f"[Subnet] index={subnet_index}, subnet={subnet}")
            ...
            with torch.no_grad():
                for batch_idx, (input, target) in enumerate(loader):
                    ...
                    if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                        log_name = '[Test batch]' + log_suffix
                        logging.info(
                            '{0} [{1:>3d}/{2}],  '
                            'loss= {loss.val:>5.4f}'.format(
                                log_name, batch_idx, last_idx,loss=losses_m))
            # import pdb;pdb.set_trace()
            metrics = OrderedDict([('loss', losses_m.avg)])
            if evaluator is not None:
                metrics['map'] = evaluator.evaluate()
            all_subnet_map.append(metrics['map'])
        print(f"[mAP] map={round(metrics['map'],4)}, index={subnet_index}, subnet={subnet}")
        subnet_index+=1
    return all_subnet_map
...
def sys_run():
    ...
    if args.local_rank == 0:
        ...
        # with open(os.path.join(output_dir, 'args_infer.yaml'), 'w') as f:
        #     f.write(args_text)
    # with open('output/config/args_train.yaml', 'w') as f:
    #     f.write(args_text)
    # print("save done")



    dir_path = "checkpoint/sample_ratio"
    ...
    for checkpoint_dir in checkpoint_list:
        ...
        recoverypath = "checkpoint/sample_ratio/" + checkpoint_dir


        if args.stage == 0:
            # model = oncloud.load_to_detectionmodel(args.backbonepath, model, part="backbone")
            if args.recovery:
                model, _ = oncloud.load_to_detectionmodel(args.headpath, model, part="head", freeze_head=True)
                model.model.load_state_dict(torch.load(recoverypath, weights_only=True))
                print("Stage=0, recovery model loaded.")
            else:
                model, _ = oncloud.load_to_detectionmodel(args.headpath, model, part="head", freeze_head=True)
                print("Stage=0, pretrained model loaded.")
        else:
            _, main_net_state = oncloud.load_to_detectionmodel(args.headpath, model, part="head", freeze_head=True)
            model.model.load_state_dict(torch.load(recoverypath, weights_only=True))
            oncloud.freeze_detection_main_subnet(model, main_net_state)
            print("Stage=1, recovery model loaded.")
        ...
        mAP_list=[]
        try:
            filename = f"output/txt/mAP_data_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".txt"
            ...
            with open(filename, "w", encoding="utf-8") as txt_file:
                for epoch in range(start_epoch, num_epochs):
                    ...
                    #     validate(model, loader_eval, args, evaluator)


                    # train_epoch(
                    #     epoch, model, loader_train, optimizer, args,sample_policy,
                    ...
                    if model_ema is not None:
                        if args.distributed and args.dist_bn in ('broadcast', 'reduce'):
                            ...
                            all_subnets_maps = validate(model_ema.module, loader_eval, args, evaluator, log_suffix=' (EMA)')
                    else:
                        all_subnets_maps = validate(model, loader_eval, args, evaluator)

                    # name = "checkpoint/SuperNet_epoch_map="+ + str(
                    #     epoch) + ".pth"  # + "loss" + str(int(train_metrics['loss'] * 1000)) + ".pth"
                    # print("====================Avg mAp====================")
                    tmp_map=""
                    for map in all_subnets_maps:
                        # print(round(map,4))
                        tmp_map+=str(round(map,4))+'\t'
                    mAP_list.append(tmp_map)

                    # name = "checkpoint/SuperNet_epoch_" + str(epoch) + "_mAP_" + f"{all_subnets_maps[0] * 100:.2f}".replace(
                    #     '.', '_') + ".pth"
                    #
                    ...
                    if lr_scheduler is not None:
                        ...
                        lr_scheduler.step(epoch + 1, all_subnets_maps[0])
                    scheduler.step()
                    ...
                    txt_file.write(f"checkpoint={checkpoint_dir.split('.')[0]}\tepoch={epoch}\tmap={tmp_map}\n")
...
if __name__ == '__main__':
    # sample_policy=["softmax","count","uniform"]
    ...
    # for checkpoint_item in checkpoint_list:
    sys_run()
