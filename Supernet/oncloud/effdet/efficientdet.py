...
class EfficientDet(nn.Module):
    ...
    def __init__(self, config, pretrained_backbone=True, alternate_init=False):
        ...
        # print(f"pretrained_backbone={pretrained_backbone}")
        ...
        # print("self.get_multi_resnet_backbone")
    ...
    def generate_random_resnet_layer(self, layerchoices, min_lat=False):
        ...
        print("layerchoices={}".format(layerchoices))
    ...
    def generate_random_subnet(self, sample_policy):
        "张俊杰：CAVE实际使用的函数"
        # print("random sample")
        # print("from train_detection.py")
        # subnets = [
        #     [[2, 99, 99, 2, 99, 99, 0], [2, 99, 99, 2, 99, 99], [2, 99, 99]],
        #     [[1, 99, 1, 99, 1, 99, 0], [1, 99, 1, 99, 1, 99], [1, 99, 0]],
        # ]
        # print("len of subnets= 10")
        # subnets = [
        ...
        if sample_policy == "softmax":
            sample_ratio = [0.398, 0.241, 0.146, 0.089, 0.054, 0.033, 0.02, 0.012, 0.007]  # softmax
        elif sample_policy == "count":
            sample_ratio = [0.2, 0.178, 0.156, 0.133, 0.111, 0.089, 0.067, 0.044, 0.022]  # count
        elif sample_policy == "uniform":
            sample_ratio = [0.12, 0.11, 0.11, 0.11, 0.11, 0.11, 0.11, 0.11, 0.11]  # uniform
        else:
            raise IOError("未识别的采样参数！")

        idx = np.random.choice(len(subnets), p=sample_ratio)
        # rand_int = random.randint(0, len(subnets)-1)
        return subnets[idx]
    ...
    def forward_resnet_backbone(self, x, subnet, distill):
        ...
        x_teacher = x.detach().clone() if distill else None
        x2, x_teacher_2, num_distill_layers_2, distill_loss_2 = self.forward_resnet_layer(self.backbone.layer2, x,
                                                                                          subnet[0], x_teacher)
        x3, x_teacher_3, num_distill_layers_3, distill_loss_3 = self.forward_resnet_layer(self.backbone.layer3, x2,
                                                                                          subnet[1], x_teacher_2)
        x4, x_teacher_4, num_distill_layers_4, distill_loss_4 = self.forward_resnet_layer(self.backbone.layer4, x3,
                                                                                          subnet[2], x_teacher_3)
